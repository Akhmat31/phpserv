...
class HostMiddlewareDecorator
{
    private string $host;
    private mixed $middleware;

    public function __construct(string $host, mixed $middleware)
    {
        $this->host = $host;
    ...
    /**
     * Process request if host matches; otherwise delegate to handler.
     */
    public function process(mixed $request, mixed $handler): Response
    {
        ...
        if (Utils::matchHost($requestHost, $this->host)) {
